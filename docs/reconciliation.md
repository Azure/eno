...
The `inventory.json` data key contains format version `1`, the source namespace, synthesizer, synthesis UUID, source `synthesized` timestamp, and resource identities with their labels and `eno.azure.io/readiness-group` / `eno.azure.io/deletion-group` annotations. Optional `sourceComposition` metadata preserves the original Composition identity and routing metadata, plus its controlling Symphony identity when present. Legacy snapshots without source metadata remain valid recovery inputs. Each inventory is limited to 1 MiB of ConfigMap data without truncation or sharding.

Selection uses the greatest source `synthesized` timestamp, not ConfigMap creation time or UUID ordering. Invalid candidates or distinct synthesis UUIDs tied at the greatest timestamp prevent selection. Missing-tombstone calculation protects currently desired resources, existing tombstones, and Eno Patch targets; recorded inventories exclude tombstones and Eno Patch pseudo-resources.
