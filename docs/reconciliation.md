...
`cmd/eno-reconciler/main.go` registers backup and reconciliation independently with the same manager. Integrations constructing the controllers directly must also register backup explicitly, even when disabled, to provide the recovery acknowledgments required by reconstitution.
