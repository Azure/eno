...
func (c *Controller) ReconcileMany(ctx context.Context, reqs []ctrl.Request) error {
	...
	if err := cmd.Run(); err != nil {
		c.logger.Error(err, "applying resources")
	}
	...
	for _, gr := range forApplication {
		...
		if cond == nil || cond.ObservedGeneration != gr.Generation {
			...
			if err != nil {
				c.logger.Error(err, "updating GR status")
			}
		}
	}

	c.logger.Info(fmt.Sprintf("cleaning up %d resources", len(forDeletion)))
	...
	if err := cmd.Run(); err != nil {
		...
		c.logger.Error(err, "deleting resources")
	}
	...
	for _, gr := range forDeletion {
		...
		if err := c.client.Update(ctx, gr); client.IgnoreNotFound(err) != nil {
			c.logger.Error(err, "removing finalizer")
		}
	}
...
func (a *accumulatingReconciler) Start(ctx context.Context) error {
	for {
		...
		a.mut.Unlock()
		a.handler(ctx, copy) // TODO: error handling w/ retries
		a.join.Broadcast()
