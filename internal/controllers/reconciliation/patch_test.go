...
// TestPatchDeletionForResourceWithReconciliation proves that a patch resource can delete the resource it references
// even the resource with reconcile-interval annotation and it won't be re-created again.
func TestPatchDeletionForResourceWithReconciliation(t *testing.T) {
	scheme := runtime.NewScheme()
	corev1.SchemeBuilder.AddToScheme(scheme)
	testv1.SchemeBuilder.AddToScheme(scheme)
	...
	// Ensure the configmap is created and reconcile at least once.
	...
	// The configmap is deleted after the patch is applied, and it won't be re-created
	// even with the reconcile interval annotation.
	err := downstream.Get(ctx, client.ObjectKeyFromObject(cm), cm)
	require.True(t, errors.IsNotFound(err))
}
