...
func (c *Controller) Reconcile(ctx context.Context, req resource.Request) (ctrl.Result, error) {
	...
	ready := c.checkReadiness(ctx, snap, current)
...
func (c *Controller) checkReadiness(ctx context.Context, snap *resource.Snapshot, current *unstructured.Unstructured) *metav1.Time {
	if snap != nil {
		status := snap.State()
	...
	readiness, ok := snap.ReadinessChecks.EvalOptionally(ctx, &apiv1.Composition{}, current)
	_, isPatch, _ := snap.Patch()
	if snap.Deleted() && !isPatch && (current == nil || snap.Orphan || snap.Disable) {
		return nil // deleting resources aren't ready until they're 404ing
	}
	if ok {
