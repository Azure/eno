...
func (c *Controller) Reconcile(ctx context.Context, req *reconstitution.Request) (ctrl.Result, error) {
	...
	if (status == nil || !status.Reconciled) && !resource.Deleted() {
		dependencies := c.resourceClient.ListReadinessGroups(ctx, synRef, resource.ReadinessGroup, -1)
...
func (c *Controller) newPatchCallback(ctx context.Context, synRef *reconstitution.SynthesisRef, resource *resource.Resource, status *apiv1.ResourceState) func() {
	return func() {
		...
		if status == nil || !status.Reconciled {
			dependants := c.resourceClient.ListReadinessGroups(ctx, synRef, resource.ReadinessGroup, 1)
